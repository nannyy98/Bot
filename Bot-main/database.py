...
class DatabaseManager:
    def __init__(self, db_path='shop_bot.db'):
        ...
        self.is_postgres = str(self.database_url).startswith(('postgres://','postgresql://'))
    ...
    def _adapt_sql(self, query: str):
        q = query
        if self.is_postgres:
            # Basic adaptations
            q = q.replace("INSERT OR IGNORE", "INSERT")
            # Ensure we DO NOTHING on conflicts (generic)
            # Append only if it's a plain INSERT and no explicit ON CONFLICT already
            if q.strip().upper().startswith("INSERT") and "ON CONFLICT" not in q.upper():
                q = q.rstrip().rstrip(";") + " ON CONFLICT DO NOTHING"
            # Replace SQLite placeholders '?' -> '%s'
            q = re.sub(r"\\?", "%s", q)
            # Skip PRAGMA statements entirely by returning a harmless select
                ...
                if os.path.exists(schema_path):
                    ...
                    for stmt in [s for s in sql.split(';') if s.strip()]:
                        try:
                            ...
                            # Ignore if table already exists or minor mismatch
                            conn.rollback()
                            cur = conn.cursor()
                            cur.execute('BEGIN;')
                            continue
                    ...
                    # Fallback: no schema file -> do nothing here
                    pass
                cur.close(); conn.close()
            except Exception as e:
                logging.error('Ошибка инициализации Postgres схемы: %s', e)
            return
        # Обновление схемы: роль пользователя
        try:
            conn_alter = self._connect()
    ...
    def execute_query(self, query, params=None):
        ...
        try:
            ...
            if params:
                ...
                cursor.execute(self._adapt_sql(query))
            q = query.strip().upper()
            if q.startswith('SELECT'):
                ...
                op = q.split()[0]
                if op == 'INSERT':
                    # Try to get inserted id in Postgres
                    if self.is_postgres and 'RETURNING' not in q:
                        try:
                            # Re-execute with RETURNING id
                            sql_ret = self._adapt_sql(query)
                            if not sql_ret.strip().upper().endswith('RETURNING ID'):
                                sql_ret = sql_ret.rstrip().rstrip(';') + ' RETURNING id'
                            if params:
                                cursor.execute(sql_ret, params)
                            else:
